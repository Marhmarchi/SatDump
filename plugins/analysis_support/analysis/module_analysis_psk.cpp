	...
	//		wave_writer.write_header(d_symbolrate, 2);
		...
		while (demod_should_run())
		{
			...
			int nout = res.process(lpf->output_stream->readBuf, dat_size, work_buffer_in);
			{
				...
				// Multiply conjugate - Normal //
				
				// Delay imaginary branch by 1 sample
				for (int i = 0; i < nout; i++)
				{
					fc32_delayed[i] = i == 0 ? fc32_last_samp : exp_output[i - 1];
					fc32_not_delayed[i] = exp_output[i];
				}
                        	
				fc32_last_samp = exp_output[nout - 1];
				...
				// Complex to Float
				volk_32fc_deinterleave_32f_x2((float *)f32_real, (float *)f32_imag, (lv_32fc_t *)work_buffer_in, nout);
                        	
				// Delay imaginary branch by 1 sample
				for (int i = 0; i < nout; i++)
				{
					f32_delayed[i] = i == 0 ? f32_last_samp : f32_imag[i - 1];
					f32_not_delayed[i] = f32_real[i];
				}
                        	
				f32_last_samp = f32_imag[nout - 1];
                        	
				// Convert the delyed imag float and (non delayed) real float to complex 
				volk_32f_x2_interleave_32fc((lv_32fc_t *)fc32, (float *)f32_not_delayed, (float *)f32_delayed, nout);
				...
				for (int i = 0; i < nout; i++)
				{
					delayed_output[i] = i == 0 ? last_samp : fc32[i - 1];
					not_delayed_output[i] = fc32[i];
				}
                        	
				last_samp = fc32[nout - 1];
